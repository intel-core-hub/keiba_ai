...
from scripts.import_approved_real_data import ALLOWED_SOURCES
from scripts.validate_live_inputs import validate_live_inputs


SANDBOX_SOURCES = {"pre_contract_sandbox", "manual_sample", "public_sample", "scrape_probe"}
...
def _source(row: dict[str, Any], default_source: str, *, kind: str) -> str:
    ...
    if source in ALLOWED_SOURCES:
        raise ValueError(f"{kind} source {source!r} is reserved for approved real-data imports")
    if source not in SANDBOX_SOURCES:
        raise ValueError(f"{kind} source must be one of {sorted(SANDBOX_SOURCES)}: {source}")
    return source
...
def _reject_canonical_output(output_root: Path) -> None:
    root = Path.cwd().resolve()
    resolved = output_root.resolve()
    for forbidden in FORBIDDEN_OUTPUT_ROOTS:
        forbidden_path = (root / forbidden).resolve()
        if resolved == forbidden_path or _is_relative_to(resolved, forbidden_path):
            raise ValueError(f"pre-contract sandbox output may not write under {forbidden}")
...
def import_pre_contract_sandbox(
    *,
    schedule_input: Path,
    odds_input: Path,
    results_input: Path,
    ...
    _reject_canonical_output(output_root)
    default_source = _source({"source": source}, source, kind="config")
    schedule_rows = _normalize_schedule(_read_csv(schedule_input), default_source)
    odds_rows = _normalize_odds(_read_csv(odds_input), default_source)
    result_rows = _normalize_results(_read_csv(results_input), default_source)
    ...
    if not validation["passed"]:
        raise ValueError("sandbox live inputs failed validation: " + "; ".join(validation["errors"]))
    return {
        "passed": True,
        ...
        "warning": "Pre-contract sandbox output is not Stage 4 evidence and must not be copied into approved_exports or canonical data paths.",
    }


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Import pre-contract sandbox data into a non-canonical live-input layout")
    parser.add_argument("--schedule-input", default="pre_contract_sandbox/schedule.csv")
    parser.add_argument("--odds-input", default="pre_contract_sandbox/odds.csv")
    parser.add_argument("--results-input", default="pre_contract_sandbox/results.csv")
    parser.add_argument("--output-root", default="reports/data_collection/pre_contract_sandbox/live_inputs")
    parser.add_argument("--source", choices=sorted(SANDBOX_SOURCES), default="pre_contract_sandbox")
    parser.add_argument("--clean", action="store_true")
    parser.add_argument("--min-races", type=int, default=1)
    parser.add_argument("--min-horses-per-race", type=int, default=2)
    parser.add_argument("--status", default="reports/data_collection/pre_contract_sandbox_import_status.json")
